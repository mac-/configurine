...
module.exports = function ConfigSvc(options) {
	...
	this.findByName = function(names, callback) {
		...
		getConfigCollection(function(error, configCollection) {
			...
			configCollection.find({name: {"$in": names} }).toArray(callback);
		});
	};
	...
	this.findByApplicationAssociation = function(appName, appVersion, callback) {
		...
		getConfigCollection(function(error, configCollection) {
			...
			configCollection.find({"associations.applications.name": appName, "associations.applications.versions": {"$in": [appVersion] } }).toArray(callback);
		});
	};
	...
	this.findByEnvironmentAssociation = function(envName, callback) {
		...
		getConfigCollection(function(error, configCollection) {
			...
			configCollection.find({"associations.environments": { "$in": [envName]} }).toArray(callback);
		});
	};
	...
	this.findByNameAndEnvironmentAssociation = function(names, envName, callback) {
		...
		getConfigCollection(function(error, configCollection) {
			...
			configCollection.find({name: {"$in": names}, "associations.environments": { "$in": [envName]} }).toArray(callback);
		});
	};
