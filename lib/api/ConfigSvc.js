...
module.exports = function ConfigSvc(options) {
	...
	var getInstrumentedCallback = function(metric, callback) {
		...
		getConfigCollection = function (callback) {
			self.connect(function(err) {
				...
				var startTime = new Date();
				self._db.collection('config', function getConfigCollectionHandler(err, collection) {
	...
	this.disconnect = function() {
		if (!self._db.serverConfig.isConnected()) {
	...
	this.connect = function(callback) {
		...
		else if (self._db._state === 'disconnected') {
			var startTime = new Date();
		...
		else if (self._db._state === 'connecting') {
