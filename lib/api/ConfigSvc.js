...
module.exports = function ConfigSvc(options) {
	...
	var getHistoryCollection = function (callback) {
		db.collection('history', function getHistoryCollectionHandler(err, collection) {
			...
			// TODO: make unique index on configId
	...
	this.findHistoryByConfigId = function(id, callback) {
		if (!id || (typeof(id) !== 'string' && typeof(id) !== 'object') || !callback || typeof(callback) !== 'function') {
			throw new Error('Missing or invalid parameters');
		}
		id = (typeof(id) === 'string') ? new ObjectID(id) : id;
		getHistoryCollection(function(error, historyCollection) {
			if (error) {
				return callback(error);
			}
			historyCollection.findOne({configId: id}, callback);
		});
	};
	...
	this.insert = function(configDoc, callback) {
		...
		getConfigCollection(function(error, configCollection) {
			...
			if (errors.length > 0) {
				return callback(new Error('A problem has occurred when validating the document: ' + JSON.stringify(errors)));
			}
			...
			configCollection.insert(configDoc, { safe: true }, function(error, newDocs) {
				...
				getHistoryCollection(function(error, historyCollection) {
					...
					historyCollection.insert({ configId: newDocs[0]._id.toString(), revisions: [{ value: configDoc, created: new Date() }] }, { safe: true, multi: false }, function(error, result) {
						if (error) {
							return callback(error);
						}
		...
		getHistoryCollection(function(error, historyCollection) {
			...
			getConfigCollection(function(error, configCollection) {
				...
				configCollection.findOne({ _id: id }, function(error, configEntry) {
					...
					// TODO: check for null config entry?
					historyCollection.update({ configId: configEntry._id.toString() }, { $push: { revisions: { value: configDoc, created: new Date() } } }, { safe: true, multi: false }, function(error, result) {
						if (error) {
							return callback(error);
						}
						configCollection.update({ _id: id }, configDoc, { safe: true, multi: false }, callback);
					});
	...
	this.restoreToRevisionNumber = function(id, index, callback) {
		if (!id || typeof(id) !== 'string' || !index || typeof(index) !== 'number' || !callback || typeof(callback) !== 'function') {
			throw new Error('Missing or invalid parameters');
		}
		getHistoryCollection(function(error, historyCollection) {
			if (error) {
			...
			getConfigCollection(function(error, configCollection) {
				...
				configCollection.findOne({ _id: id }, function(error, configEntry) {
					...
					// TODO: check for null config entry?
					historyCollection.find({ configId: configEntry._id.toString() }, { safe: true, multi: false }, function(error, historyDoc) {
						if (error) {
							return callback(error);
						}
						if (historyDoc.revisions.length <= index) {
							return callback(new Error('Error: Revision number ' + index + ' does not exist.'));
						}
						var configDoc = historyDoc.revisions[index].value;
						historyCollection.update({ configId: configEntry._id.toString() }, { $push: { revisions: { value: configDoc, created: new Date() } } }, { safe: true, multi: false }, function(error, result) {
							if (error) {
								return callback(error);
							}
		...
		getHistoryCollection(function(error, historyCollection) {
			...
			getConfigCollection(function(error, configCollection) {
				...
				configCollection.findOne({ _id: id }, function(error, configEntry) {
					...
					// TODO: check for null config entry?
					historyCollection.remove({ configId: configEntry._id.toString() }, { safe: true }, function(error, result) {
						if (error) {
							return callback(error);
						}
	...
	this.removeAll = function(callback) {
		...
		getHistoryCollection(function(error, historyCollection) {
			...
			getConfigCollection(function(error, configCollection) {
				...
				historyCollection.remove({ }, { safe: true }, function(error) {
					if (error) {
						return callback(error);
					}
					configCollection.remove({ }, { safe: true }, callback);
				});
