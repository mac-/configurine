...
module.exports = function AuthenticationMgr(options) {
	...
	this.addSystemClient = function(name, sharedKey, email, applications, callback) {
	...
	this.addUserClient = function(name, password, email, applications, callback) {
		if (!name || typeof(name) !== 'string' || !password || typeof(password) !== 'string' || !email || typeof(email) !== 'string' ||
			!applications || typeof(applications) !== 'object' || !callback || typeof(callback) !== 'function') {
			throw new Error('Missing or invalid parameters');
		}
		var startTime = new Date();
