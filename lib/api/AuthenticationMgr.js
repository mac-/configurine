...
module.exports = function AuthenticationMgr(options) {
	...
	var logger = options.logger || {log:function(){},trace:function(){},debug:function(){},info:function(){},warn:function(){},error:function(){}},
		...
		ensureAdminExists = function() {
			clientSvc.findAll(function(err, allClients) {
				...
				console.log('!! NO ADMIN EXISTS IN THE SYSTEM. CREATING NEW ADMIN...');
				...
				clientSvc.insert(client, function(err, result) {
					...
					console.log('!! ADMIN CREATED: CLIENT ID: ' + client.name + '   KEY: ' + client.privateKey);
					console.log('!! STORE THIS INFO IN A SAFE PLACE. THIS INFO WILL NOT BE DISPLAYED AGAIN.');
				});
	...
	this.updateClient = function(name, email, isConfirmed, isAdmin, callback) {
		...
		clientSvc.findByName(name.toLowerCase(), function(err, existingClient) {
			...
			if (!existingClient) {
				return callback(new Error('Not FoundError: ' + name.toLowerCase() + ' does not exist in the system'));
			}
	...
	this.removeClient = function(name, callback) {
		...
		clientSvc.removeByName(name.toLowerCase(), function(err, result) {
			...
			if (!result) {
				return callback(new Error('Not Found Error: ' + name.toLowerCase() + ' does not exist in the system'));
			}
			callback(null, result);
		});
	};
