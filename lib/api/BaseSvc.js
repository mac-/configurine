...
module.exports = function BaseSvc(options) {
	...
	var isConnecting = false,
		...
		dotReplacementRegex = new RegExp(dotReplacement, 'g'),
	...
	this._getInstrumentedCallback = function(metric, cb) {
		...
		return function(err, result) {
			...
			self._decodeDotsInPropertyNames(result);
			cb(err, result);
		};
	};

	this._encodeDotsInPropertyNames = function(obj) {
		var prop, newProp;
		for (prop in obj) {
			if (obj.hasOwnProperty(prop)) {
				if (typeof(obj[prop]) === 'object') {
					self._encodeDotsInPropertyNames(obj[prop]);
				}
				if (prop.indexOf('.') >= 0) {
					newProp = prop.replace(/\./g, dotReplacement);
	...
	this._decodeDotsInPropertyNames = function(obj) {
		var prop, newProp;
		for (prop in obj) {
			if (obj.hasOwnProperty(prop)) {
				if (typeof(obj[prop]) === 'object') {
					self._decodeDotsInPropertyNames(obj[prop]);
				}
				if (prop.indexOf(dotReplacement) >= 0) {
					newProp = prop.replace(dotReplacementRegex, '.');
					obj[newProp] = obj[prop];
					delete obj[prop];
				}
			}
	...
	this.insert = function(doc, callback) {
		...
		self._getCollection(function(error, collection) {
			...
			self._encodeDotsInPropertyNames(doc);
